...
assert.equal(run("state['assay.refInjection']"), '1', '固定参数以标准值为准');
assert.equal(run("state['assay.f.1']"), '50');
assert.equal(run("state['assay.sampleInjection.2']"), '1');
run("var edited = {'assay.f.1':'99'}; GcPubiaoDefaults.fill('mint-menthol', edited, GC_WORD_TABLE_LAYOUTS['mint-menthol']);");
assert.equal(run("edited['assay.f.1']"), '50', '固定参数不接受改值');
...
assert.equal(run("shanghai['assay.refInjection']"), '1', '上海2018进样量按药典砂仁同法制样固定为1');
...
assert.equal(run("beijing['assay.sampleInjection.1']"), '1', '北京2023进样量按药典砂仁同法制样固定为1');
