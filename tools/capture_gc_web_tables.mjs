    ...
    for (const templateId of ids) {
      ...
      for (const role of ['reference', 'sample']) {
        ...
        all[role] = await table.evaluate((element, roleName) => {
          ...
          [...element.rows].forEach((row, r) => { let col = 0; while (occupied[r]?.[col]) col++; [...row.cells].forEach(cell => { while (occupied[r]?.[col]) col++; const rect = cell.getBoundingClientRect(), style = getComputedStyle(cell), rowSpan = cell.rowSpan, gridSpan = cell.colSpan, lines = textLines(cell); for (let rr = r; rr < r + rowSpan; rr++) { occupied[rr] ||= []; for (let cc = col; cc < col + gridSpan; cc++) occupied[rr][cc] = true; } cells.push({ id: `r${r + 1}c${col + 1}`, rowIndex: r + 1, gridColumnIndex: col + 1, rowSpan, gridSpan, x: rect.left - tableRect.left, y: rect.top - tableRect.top, width: rect.width, height: rect.height, text: cell.innerText, textLineCount: lines.count, textLineRects: lines.rects, textLineBoxes: lines.boxes, className: cell.className, borders: Object.fromEntries(['top','right','bottom','left'].map(side => [side, border(style, side)])) }); col += gridSpan; }); });
