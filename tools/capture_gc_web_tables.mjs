  ...
  for (const templateId of ids) {
    ...
    for (const role of ['reference', 'sample']) {
      ...
      all[role] = await table.evaluate((element, roleName) => {
        ...
        const textLines = cell => { const range = document.createRange(); range.selectNodeContents(cell); const ys = [...range.getClientRects()].filter(rect => rect.width && rect.height).map(rect => rect.top - tableRect.top).sort((a,b) => a-b); return ys.reduce((values, y) => !values.length || Math.abs(y - values.at(-1)) > .75 ? [...values, y] : values, []).length; };
