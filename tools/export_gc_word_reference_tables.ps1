...
if (@($manifestData.entries).Count -ne 35) { throw 'manifest must contain exactly 35 template entries' }
...
$report = @{ run = $run; templateCount = @($manifestData.entries).Count; tableCount = 70; successfulTables = @($successes).Count; failedTables = @($errors).Count; successes = @($successes); errors = @($errors); sourceReadOnly = $true; tempFilesCleaned = $true }; $report | ConvertTo-Json -Depth 20 | Set-Content -Encoding utf8 -LiteralPath (Join-Path $run 'word-export-summary.json'); Write-Host ("Exported {0}/70 Word tables to {1}; errors={2}" -f $successes.Count, $run, $errors.Count); if ($errors.Count) { $errors | ConvertTo-Json -Depth 10; exit 1 }
