...
function Crop-VisibleTablePng([string]$sourcePng, [string]$destinationPng) {
  ...
  try {
    ...
    return [pscustomobject]@{ cropPx144 = @{ width = $rect.Width; height = $rect.Height }; cropCssPx = @{ width = [Math]::Round($rect.Width * 2 / 3, 3); height = [Math]::Round($rect.Height * 2 / 3, 3) }; fullPagePx144 = @{ width = $bitmap.Width; height = $bitmap.Height } }
  } finally { if ($crop) { $crop.Dispose() }; $bitmap.Dispose() }
}
function Export-Table($word, $sourcePath, [int]$tableIndex, $role, $targetDir, $sourceStructure) {
...
try { $word = New-Object -ComObject Word.Application; $word.Visible = $false; $word.DisplayAlerts = 0
  foreach ($entry in @($manifestData.entries)) { $target = Join-Path $run $entry.templateId; New-Item -ItemType Directory -Force -Path $target | Out-Null; $source = Join-Path (Join-Path $RecordsBase $entry.root) $entry.sourceFile
    foreach ($role in @(@{ name = 'reference'; index = [int]$entry.referenceTableIndex }, @{ name = 'sample'; index = [int]$entry.sampleTableIndex })) { try { $meta = Export-Table $word $source $role.index $role.name $target $structureByKey["$($entry.templateId)|$($role.name)"]; $successes.Add(@{ templateId = $entry.templateId; role = $role.name; sourceTableIndex = $role.index; cropCssPx = $meta.crop.cropCssPx }) } catch { $errors.Add(@{ templateId = $entry.templateId; role = $role.name; sourceTableIndex = $role.index; error = $_.Exception.Message }) } }
  }
